...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gipmds="http://www.gip-mds.fr/">
					...
					<tbody>
						...
						<tr>
							...
							<td class="templateCorps" rowspan="2" valign="top" width="100%">
								...
								<xsl:if test="gipmds:rapport/declaration">	
									<xsl:for-each select="gipmds:rapport/declaration">
										...
										<div class="templateTableauConteneur">
											...
											<xsl:if test="declaration_bilan/salarie">
												<br/>
												<table width="800">
													<tr>
								...
								<xsl:if test="gipmds:rapport/communication">
									<br />
									<div class="templateSection">COMMUNICATION</div>
									<xsl:apply-templates select="gipmds:rapport/communication/information_collective"/>
									<br/>
								</xsl:if>
	...
	<xsl:template match="declaration_anomalie|envoi_anomalie">
		...
		<xsl:if test="environnement">
			...
			<table width="800">
				<tr>
					<xsl:if test="environnement/salarie|environnement/individu_non_salarie">
	...
	<xsl:template match="information_collective">
		<br/>	
		<div class="templateBlocETBContenu">
			<span class="templateBlocETBImportant"><strong>Informations de nature collective<xsl:if test="@type"> de type "<xsl:value-of select="@type"/>"</xsl:if></strong></span>	
			<br />
			<xsl:apply-templates select="calcul"/>
		</div>
		<br/>	
	</xsl:template>
	
	<xsl:template match="calcul">
		<div class="templateTitreInformation">Calcul des informations de nature collective :</div>	
		<xsl:if test="@date_calcul">
	...
	<xsl:template match="information">
		<br/>	
		<div class="templateTitreInformation">Informations :</div>
		<xsl:if test="donnees_clefs">
			<strong>Données clefs : </strong>
			<br/>	
			<xsl:apply-templates select="donnees_clefs/rubriques"/>
			<br/>
		</xsl:if>
		<xsl:if test="donnees_paie">
			<strong>Données de paie : </strong>
			<br/>	
			<xsl:if test="@tranche">
				<u>Tranche</u> : 
			...
			<xsl:if test="donnees_paie/taux">
				<u>Taux <i><xsl:value-of select="donnees_paie/taux/@part"/></i></u> : 
				<xsl:value-of select="donnees_paie/taux"/>
				<br/>
			</xsl:if>
			<xsl:if test="donnees_paie/montant">
				<u>Montant <i><xsl:value-of select="donnees_paie/montant/@type"/></i></u> : 
				<xsl:value-of select="donnees_paie/montant"/>
				<br/>
			</xsl:if>
			<xsl:apply-templates select="donnees_paie/rubriques"/>
			<br/>
		</xsl:if>
		<xsl:if test="donnees_declaration">
			<strong>Données de déclaration : </strong>
			<br/>	
			<xsl:apply-templates select="donnees_declaration/rubriques"/>
			<br/>
		</xsl:if>
		<xsl:if test="donnees_complement">
			<strong>Données complémentaires : </strong>
			<br/>	
			<xsl:apply-templates select="donnees_complement/rubriques"/>
			<br/>
		</xsl:if>
	</xsl:template>
